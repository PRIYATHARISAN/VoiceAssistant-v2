...
def _create_httpx_client(timeout: float | httpx.Timeout) -> httpx.AsyncClient:
    """Create httpx.AsyncClient with proxy bypass protection for local intranet hostnames."""
    no_proxy = os.environ.get("NO_PROXY") or os.environ.get("no_proxy") or ""
    bypasses = ["karunya.edu", ".karunya.edu", "localhost", "127.0.0.1"]
    existing = [p.strip() for p in no_proxy.split(",") if p.strip()]
    added = False
    for b in bypasses:
        if b not in existing:
            existing.append(b)
            added = True
    if added or not no_proxy:
...
async def chat_completion(
    ...
    for attempt in range(_STREAM_RETRY_ATTEMPTS):
        try:
            async with _create_httpx_client(httpx.Timeout(15)) as client:
            ...
            if attempt == _STREAM_RETRY_ATTEMPTS - 1:
                raise
            logger.warning(
                "[chat_completion] transient completion failure (%s); retrying (%s/%s)",
                base_url,
                attempt + 1,
...
async def stream_anthropic(
    ...
    for attempt in range(_STREAM_RETRY_ATTEMPTS):
        try:
            async with _create_httpx_client(_STREAM_TIMEOUT) as client:
