...
class AuthMode(str, Enum):
    NONE = "none"  # Local, unauthenticated
...
async def get_or_create_user(username: str, default_role: str = "user") -> str:
    ...
    async with await get_db() as db:
        ...
        if not auth:
            user = User(created_at=now_ms(), role=default_role)
            db.add(user)
...
def get_auth_mode() -> AuthMode:
    ...
    from cptr.env import LOCAL_AUTH_DISABLED
...
def check_access(
    ...
    if mode == AuthMode.NONE:
        return AuthResult(user_id="local_admin", username="local_admin", role="admin")
    ...
    if mode == AuthMode.TRUSTED_HEADER:
        ...
        if remote_user_header:
            return AuthResult(user_id=remote_user_header, username=remote_user_header)
        if jwt_token:
            return verify_token(jwt_token)
        return None
