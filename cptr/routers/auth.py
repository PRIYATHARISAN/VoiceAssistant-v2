...
async def get_auth_config():
    """Public config endpoint for frontend (alias for /api/config)."""
    mode = get_auth_mode()
    needs_setup = False
...
async def get_auth(request: Request):
    ...
    if auth is not None and auth.username and (not auth.user_id or auth.user_id == "local_admin"):
        auth.user_id = await get_or_create_user(auth.username, default_role=auth.role)

    if auth is not None and auth.user_id:
        if not auth.exp:
            auth.exp = time.time() + SESSION_MAX_AGE
        user = await User.get_by_id(auth.user_id)
        if user is None and get_auth_mode() == AuthMode.NONE:
            auth.user_id = await get_or_create_user(auth.username or "local_admin", default_role=auth.role)
            user = await User.get_by_id(auth.user_id)
        ...
        if user is None:
            if get_auth_mode() == AuthMode.NONE:
                return {
                    "authenticated": True,
                    "user_id": auth.user_id or "local_admin",
                    "username": auth.username or "local_admin",
                    "display_name": "Local Admin",
                    "role": "admin",
                    "profile_image_url": None,
                    "exp": int((time.time() + SESSION_MAX_AGE) * 1000),
                }
        ...
        data = {
            ...
            "username": auth.username or "local_admin",
            "display_name": user.display_name or auth.username or "local_admin",
        ...
        if get_auth_mode() != AuthMode.NONE:
            ...
            if remote_user or remaining < SESSION_MAX_AGE / 2:
                new_token = create_token(auth.user_id, auth.username or "local_admin", user.role)
...
async def setup(request: Request, body: SetupRequest):
    ...
    if expected and body.token:
        if not _secrets.compare_digest(body.token, expected):
