...
async def _get_connections() -> list[dict]:
    conns = await Config.get("chat.connections") or []
    sofie_key = os.getenv("SOFIE_API_KEY") or await Config.get("chat.sofie_api_key")
    if sofie_key:
        has_sofie = any(c.get("prefix_id") == "sofie" or c.get("id") == "sofie" for c in conns)
        if not has_sofie:
            sofie_base = os.getenv("SOFIE_BASE_URL") or "https://llm.karunya.edu/v1"
            sofie_model = os.getenv("SOFIE_MODEL") or "sofie-code"
            conns.append({
                "id": "sofie",
                "name": "Sofie Code LLM",
                "prefix_id": "sofie",
