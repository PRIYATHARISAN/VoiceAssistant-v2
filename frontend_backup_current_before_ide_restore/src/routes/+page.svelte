<script lang="ts">
	...
	import ExcelView from '$lib/components/ExcelView.svelte';
	...
	$effect(() => {
		if (typeof window === 'undefined') return;
		const handleNewChatFromHome = (e: Event) => {
			const detail = (e as CustomEvent).detail;
			if (detail?.text) {
				if ($currentWorkspace) {
					const key = `cptr:intent:chatDraft:${$currentWorkspace.path}`;
					sessionStorage.setItem(key, detail.text);
				}
				activeView.set('excel');
			}
		};
		const handleExcelCommand = () => {
			activeView.set('excel');
		};

		window.addEventListener('cptr:new-chat-from-home', handleNewChatFromHome);
		window.addEventListener('cptr:excel-command', handleExcelCommand);
		return () => {
			window.removeEventListener('cptr:new-chat-from-home', handleNewChatFromHome);
			window.removeEventListener('cptr:excel-command', handleExcelCommand);
		};
	});
